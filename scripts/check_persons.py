...
try:
    with engine.connect() as conn:
        ...
        if 'persons' in tables:
            ...
            sample = conn.execute(text('SELECT id, first_name, last_name, email FROM persons ORDER BY id LIMIT 5')).fetchall()
        ...
        cols = conn.execute(text("SELECT column_name FROM information_schema.columns WHERE table_name='users' AND table_schema='public';")).fetchall()
        cols = [c[0] for c in cols]
        print('users columns:', cols)
        if 'person_id' in cols:
            nulls = conn.execute(text('SELECT COUNT(*) FROM users WHERE person_id IS NULL')).scalar()
            total = conn.execute(text('SELECT COUNT(*) FROM users')).scalar()
            print(f'users total={total}  person_id NULLs={nulls}')
            samples = conn.execute(text('SELECT id, email, person_id FROM users ORDER BY id LIMIT 10')).fetchall()
